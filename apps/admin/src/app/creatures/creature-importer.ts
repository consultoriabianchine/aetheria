...
export class CreatureImporter {
  ...
  editCreature(field: 'name' | 'slug' | 'description' | 'hp' | 'experience' | 'armor' | 'charms' | 'difficulty', value: unknown) {
    this.preview.update((current) => {
      if (!current) return current;
      const nextValue = ['hp', 'experience', 'armor', 'charms'].includes(field) ? this.numberValue(value) : String(value ?? '');
  ...
  private lootValue(field: string, value: unknown): unknown {
    if (['chance', 'minQuantity', 'maxQuantity', 'weight', 'attackPower', 'armor', 'defense', 'sellValue'].includes(field)) return this.numberValue(value) ?? 0;
