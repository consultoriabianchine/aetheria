...
import type { EffectTypeDefinition, ItemImpactVisual, ItemVisualEffects } from '@aetheria/types';
...
@Component({
  selector: 'admin-effect-type-editor',
  imports: [FormsModule, SpriteVisualEditor],
  templateUrl: './effect-type-editor.html',
  styles: `
...
export class EffectTypeEditor implements OnInit {
  readonly items = signal<EffectTypeDefinition[]>([]);
  ...
  async load() {
    this.error.set(null);
    try {
      this.items.set(await this.api.listEffectTypes());
  ...
  select(item: EffectTypeDefinition) {
    ...
    this.draft.set({
      ...
      visual: { impact: structuredClone(item.impact) },
    });
  }
  ...
  patchVisual(visual: ItemVisualEffects | null) {
    this.patch({ visual: visual ?? { impact: blankImpact() } });
  }
  ...
  async save() {
    ...
    this.error.set(null);
    try {
      const p = draft.visual.impact ?? blankImpact();
      await this.api.saveEffectType({
        id: draft.id,
        slug: draft.slug,
        name: draft.name,
...
function blank(): Draft {
  return { slug: '', name: 'Novo Tipo de Efeito', description: '', enabled: true, visual: { impact: blankImpact() } };
}
